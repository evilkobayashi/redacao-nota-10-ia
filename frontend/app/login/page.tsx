...
import { PenTool, Mail, Lock, ArrowLeft, Eye, EyeOff } from 'lucide-react'
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    // TODO (Gemini): Conectar ao Supabase Auth signInWithPassword({ email, password })
    setLoading(true)
    setMessage({ text: 'Autenticação será conectada pelo backend.', type: 'info' })
    setLoading(false)
  }
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-12 bg-white font-sans text-gray-900 overflow-x-hidden">
      ...
      <div className="hidden lg:flex lg:col-span-6 relative bg-gradient-to-br from-red-900 via-red-950 to-zinc-950 p-12 flex-col justify-between overflow-hidden border-r border-red-900/50">
        ...
        <div className="relative z-10 space-y-6 my-auto max-w-lg">
          <span className="inline-flex items-center gap-2 bg-red-500/20 text-red-300 border border-red-400/30 text-xs font-extrabold px-3.5 py-1.5 rounded-full uppercase tracking-wider">
            Correção ENEM com IA
          </span>
          <h2 className="text-4xl lg:text-5xl font-black text-white leading-tight">
            Descubra sua nota antes da prova.
          </h2>
          <p className="text-gray-300 text-base leading-relaxed">
            A única IA treinada nos manuais oficiais do INEP. Feedback real nas 5 competências em menos de 30 segundos.
          </p>
        </div>
      ...
      <div className="lg:col-span-6 flex flex-col justify-between p-6 sm:p-12">
        ...
        <div className="w-full max-w-md mx-auto my-auto py-6">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-extrabold text-gray-950 tracking-tight">Bem-vindo de volta</h1>
            <p className="text-sm text-gray-500 mt-2">Entre para continuar corrigindo suas redações.</p>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-xl">
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              Não tem conta?{' '}
              <Link href="/signup" className="font-bold text-red-600 hover:underline">Criar conta grátis</Link>
            </p>
          </div>
        </div>
